...
def load_data(args):
    ...
    #     pass
    ...
    elif "Friday" in args.dataset or "Thursday" in args.dataset or "Wednesday" in args.dataset:
        filename = f'{data_dir}{args.dataset}-2018_processed.csv'
    ...
    elif "diabetes" in args.dataset:
        filename = f'{data_dir}{args.dataset}.csv'
        data = pd.read_csv(filename)
        y = data['label'].values
        X = data.drop(columns=['label'])
        print(X)
        ...
        print(f"{args.dataset} not recognized")
