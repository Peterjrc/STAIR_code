...
from main_original import calculate, traverse
...
def draw_cluster(cluster, color):
    sns.scatterplot(x=cluster[:, 0], y=cluster[:, 1] , c=[color] * len(cluster), sizes=5000)
...
class LocalDT():
    ...
    def run(self, X, y, threshold, gap=0.2):
        ...
        while True:
            ...
            # self.update_decision_tree(X, y, threshold=0.6 + 0.2 / 10 * (i - 2))
    ...
    def draw(self, X):
        ...
        plt.savefig(f"./experiment_data/cluster_lstair/{name}_cluster.pdf", format='pdf', dpi=600)
        plt.show()
...
def main(name, num, data_dir, threshold, gap):
    ...
    number, length, n_c = local_lime.run(X_transformed, lof_predictions, threshold, gap=gap)

    print('#####drawing clusters produced by L-STAIR####')
    # local_lime.draw(X_transformed)
    # np.save(f"./{name}.npy", np.array(local_lime.clusters))
    # from sklearn.manifold import TSNE
    # X_embed = TSNE(n_components=2).fit_transform(X_transformed)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == 'All':
            datasets = ['Pendigits', "PageBlock", "satellite", "Satimage-2", "Musk", "ALOI", "KDDCup99",
            "optdigits", "Annthyroid"]
        else:
            datasets = [sys.argv[1]]
    else:
        datasets = ['Shuttle']

    # max_lengths = [4, 6, 8, 10, 12]
    max_lengths = [10]
    numbers = [2,4,8]
    # numbers = [2]
    ...
    df_ml = pd.DataFrame(columns=datasets, index=max_lengths)
    results = []
    ...
    df_ml.to_csv('effect_lm_lstair.csv')
