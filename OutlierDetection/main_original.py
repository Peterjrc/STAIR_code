...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        if sys.argv[1] == 'All':
            names = ['Pendigits', 'PageBlock', "Shuttle", "Pima", "Mammography", "Satimage-2", "satellite",
    ...
    max_lengths = [10]
    # methods = ['ID3', 'CART', 'RIPPER']
    methods = ["MDT"]
    # methods = ['RIPPER']
    thresholds = [0.8]
    # thresholds = [0.7, 0.75, 0.8, 0.85, 0.95]
    results = []
    result_df = pd.DataFrame(columns=methods)
    ...
    for name in names:
        ...
        def run_lof(X, y, num_outliers=560, k=60):
            ...
            print(f"F-1 score of LOF outlier detection for {name}:", metrics.f1_score(y, lof_predictions))
            print(f"Recall score of LOF outlier detection for {name}:", metrics.recall_score(y, lof_predictions))
            return lof_predictions, lof_scores


        def run_isolation_forest(X, y, num_outliers = 560, max_features=1.0):
            # training the model
            ...
            threshold = np.sort(if_scores)[::-1][num_outliers]
            if_predictions = np.array(if_scores > threshold)
            if_predictions = np.array([int(i) for i in if_predictions])
            f1_score = metrics.f1_score(y, if_predictions)
            print(f"F-1 score of Isolation Forest outlier detection for {name}:", f1_score)
            return if_predictions, if_scores


        def run_knn(X, y, num_outliers=560, k=60):
            neigh = NearestNeighbors(n_neighbors=k)
            neigh.fit(X)
            knn_dists = neigh.kneighbors(X)[0][:, -1]
            threshold = np.sort(knn_dists)[::-1][num_outliers]
            knn_predictions = np.array(knn_dists > threshold)
            knn_predictions = np.array([int(i) for i in knn_predictions])
            f1_score = metrics.f1_score(y, knn_predictions)
            print(f"F-1 score of KNN outlier detection for {name}:", f1_score)
            return knn_predictions, knn_dists


        def run_mahalanobis(x, num_outliers=560):
            ...
            mahalanobis_dist = np.array(results)
            threshold = np.sort(mahalanobis_dist)[::-1][num_outliers]
            mahalanobis_predictions = np.array(mahalanobis_dist > threshold)
            mahalanobis_predictions = np.array([int(i) for i in mahalanobis_predictions])
            f1_score = metrics.f1_score(y, mahalanobis_predictions)
            print(f"F-1 score of mahalanobis outlier detection for {name}:", f1_score)
            return mahalanobis_predictions, mahalanobis_dist


        # run lof outlier detector
        lof_predictions, lof_scores = run_lof(X, y, k=lof_krange[7], num_outliers=int(np.sum(y)))

        # run isolation forest detector
        # if_predictions, if_scores = run_isolation_forest(X, y, num_outliers=int(np.sum(y)))

        # run knn outlier detector
        # knn_predictions, knn_scores = run_knn(X, y, k=knn_krange[7], num_outliers=int(np.sum(y)))

        # run mahalanobis outlier detector
        # mahalanobis_predictions, mahalanobis_scores = run_mahalanobis(X, num_outliers=int(np.sum(y)))
            ...
            for ratio in [1.0]:
                for max_length in max_lengths:
                    for threshold in thresholds:
                        ...
                        nums = []
                        for i in range(1):
                            # run stair on lof predictions
                            result = main(name, lof_predictions, lof_scores, ratio, max_length, threshold, method)

                            # run stair on isolation forest predictions
                            # result = main(name, if_predictions, if_scores, ratio, max_length, threshold, method)

                            # run stair on knn predictions
                            # result = main(name, knn_predictions, knn_scores, ratio, max_length, threshold, method)

                            # run stair on mahalanobis predictions
                            # result = main(name, mahalanobis_predictions, mahalanobis_scores, ratio, max_length, threshold, method)
                        ...
                        result_df.loc[name, method] = np.mean(lengths)
                        print(results)
                        print(result_df)
