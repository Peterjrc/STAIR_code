...
class ModelEntropyEarlyStop:  # version of saving parameters
    ...
    def step(self,eval_loader,model, data_length):
        if self.isStop:
            return True

        loss_function = nn.CrossEntropyLoss()
        loss_function_raw = nn.CrossEntropyLoss(reduction='none')
        with torch.no_grad():
            num = 0
            loss_list = np.zeros((data_length, ), dtype=np.float64) # store the loss for each instance in eval set
            model.eval()
            for data in eval_loader:
                idx, eval_feature, eval_label = data
                eval_feature = eval_feature.to(device)
                eval_label = eval_label.to(device)
                eval_label_predict = model(eval_feature)

                loss = loss_function_raw(eval_label_predict, eval_label).detach().cpu().numpy()
                loss_list[idx] = loss
                num += 1
            eval_score = loss_list
            entropy_value = cal_entropy(eval_score) # calculate the loss entropy
            model.train()
